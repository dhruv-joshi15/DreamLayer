...
const Txt2ImgPage: React.FC<Txt2ImgPageProps> = ({ selectedModel, onTabChange }) => {
  ...
  const handlePromptChange = (value: string, isNegative = false) => {
  ...
  const handleSamplingSettingsChange = (sampler: string, scheduler: string, steps: number, cfg: number) => {
    updateCoreSettings({ sampler_name: sampler, scheduler, steps, cfg_scale: cfg });
  };
  ...
  const handleSeedChange = (seed: number, random = true) => {
    ...
    <div className={`mb-4 ${isMobile ? 'grid grid-cols-1' : 'grid gap-6 md:grid-cols-[1.8fr_1fr]'}`}>
      <div className="space-y-4">
        <div className="flex flex-col">
          ...
          {isMobile && <ImagePreview onTabChange={onTabChange} />}

          <Accordion title={getAccordionTitle()} number="1" defaultOpen={true}>
            {activeSubTab === 'checkpoints' ? (
              <CustomWorkflowBrowser onWorkflowChange={setCustomWorkflow} currentWorkflow={customWorkflow} />
            ) : activeSubTab === 'lora' ? (
              <LoraBrowser />
            ) : (
              <>
                <PromptInput label="Prompt" placeholder="Enter your prompt here" value={coreSettings.prompt} onChange={v => handlePromptChange(v)} />
                <PromptInput label="Negative Prompt" negative={true} placeholder="Enter negative prompt here" value={coreSettings.negative_prompt} onChange={v => handlePromptChange(v, true)} />
                <RenderSettings sampler={coreSettings.sampler_name} scheduler={coreSettings.scheduler} steps={coreSettings.steps} cfg={coreSettings.cfg_scale} onChange={handleSamplingSettingsChange} />
            ...
            <>
              <Accordion title="Advanced Optional Settings" number="2">
                <AdvancedSettings
                  ...
                  onRestoreFacesChange={v => updateCoreSettings({ restore_faces: v })}
                  faceRestorationModel={coreSettings.face_restoration_model}
                  onFaceRestorationModelChange={v => updateCoreSettings({ face_restoration_model: v })}
                  codeformerWeight={coreSettings.codeformer_weight}
                  onCodeformerWeightChange={v => updateCoreSettings({ codeformer_weight: v })}
                  gfpganWeight={coreSettings.gfpgan_weight}
                  onGfpganWeightChange={v => updateCoreSettings({ gfpgan_weight: v })}
                  tiling={coreSettings.tiling}
                  onTilingChange={v => updateCoreSettings({ tiling: v })}
                  tileSize={coreSettings.tile_size}
                  onTileSizeChange={v => updateCoreSettings({ tile_size: v })}
                  overlap={coreSettings.tile_overlap}
                  onOverlapChange={v => updateCoreSettings({ tile_overlap: v })}
                  hiresFix={coreSettings.hires_fix}
                  onHiresFixChange={v => updateCoreSettings({ hires_fix: v })}
                  refinerEnabled={coreSettings.refiner_enabled}
                  onRefinerEnabledChange={v => updateCoreSettings({ refiner_enabled: v })}
                  refinerModel={coreSettings.refiner_model}
                  onRefinerModelChange={v => updateCoreSettings({ refiner_model: v })}
                  refinerSwitchAt={coreSettings.refiner_switch_at}
                  onRefinerSwitchAtChange={v => updateCoreSettings({ refiner_switch_at: v })}
                />
              </Accordion>
